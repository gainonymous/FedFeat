...
def train_client_fedavg(id, client_loader, global_model, num_local_epochs, lr, device):
...
def train_client_fedavg_fedfeat(id, client_loader, global_model, num_local_epochs, lr, device):
    local_model = copy.deepcopy(global_model)
...
def fed_avg_experiment(global_model, num_clients_per_round, num_local_epochs, lr, client_train_loader, max_rounds, filename, test_loader, device):
    round_accuracy = []
    for t in range(max_rounds):
        ...
        for i,c in enumerate(clients):
            ...
            local_model = train_client_fedavg(c, client_train_loader[c], global_model, num_local_epochs, lr, device)
...
def fed_avg_fedfeat_experiment(global_model, num_clients_per_round, num_local_epochs, lr, client_train_loader, max_rounds, filename, test_loader, device, num_global_epochs, global_lr):
    round_accuracy = []
    for t in range(max_rounds):
        print("starting round {}".format(t))
        ...
        for i,c in enumerate(clients):
            # train local client
            print("round {}, starting client {}/{}, id: {}".format(t, i+1,num_clients_per_round, c))
            local_model, features_list, labels_list = train_client_fedavg_fedfeat(c, client_train_loader[c], global_model, num_local_epochs, lr, device)
